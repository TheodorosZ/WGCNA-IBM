...
```{r}
setwd("C:\\Users\\thodw\\Downloads\\Systems Biology 2020-2021\\Master Thesis\\DESeq2 output dataset")
...
```{r}
gsg = goodSamplesGenes(raw_counts, verbose = 3);
gsg$allOK
```

```{r}
# normalized counts from RNA-seq data should be log-transformed
data.log <- log2(raw_counts+1)
data.log <- as.data.frame(t(as.matrix(data.log)))
```

```{r}
traitData = read.csv("Sample_metadata_final.csv", header=T)
traitData <- traitData %>% select(-name)
traitData <- traitData %>% select(-Prev_sampleID)

data.filtered.dcm = data.log[row.names(data.log) %in% traitData$ID,]
...
datTraits[datTraits=="Control_Amputee"]<-2
...
net = blockwiseModules(data.filtered.dcm, power = 8, maxBlockSize = 5000, 
...
TOM = TOMsimilarityFromExpr(data.filtered.dcm, power = 8);
